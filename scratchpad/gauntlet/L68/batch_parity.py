"""L68: does e1_eval.run_batch (one model forward per round across N matches) reproduce the sequential run_match?

    research/ext/Royale/.venv/Scripts/python.exe scratchpad/gauntlet/L68/batch_parity.py [--n 29] [--device cuda]

Reference = the ranking check's v6aug_s1 records (L68/rank, sequential, CPU). A batched run can differ only through
float noise in a batched forward (padding is fixed at 64 units, so shapes match), which could flip a near-tie argmax.
"""
import argparse
import json
import sys
...
ref = {}
for f in (REPO / "scratchpad/gauntlet/L68/rank").glob("v6aug_s1_sh*/matches.jsonl"):
...
E.run_batch(lambda: RoyalePoolEnv(subs={"Tornado": "Arrows"}), model, load_deck("icebow"), jobs, cfg, a.n,
