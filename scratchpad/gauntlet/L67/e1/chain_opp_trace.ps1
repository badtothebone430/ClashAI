# chain_opp_trace.ps1 -- run the OpponentElixirEstimator charge-event TRACE (O10's pipeline/opp_est_audit.py
# --charge-trace) over 5 held-out ghosts.
#
...
# view), driven through the same S1 live policy / engine match loop e1_eval uses. Entries 0:5 of the heldout
# split (pool v1), same ckpt/port/seed/shard settings as the rest of this batch, PLUS --charge-trace so
# condition A also writes <out>/charges.jsonl and <out>/track_events.jsonl (module docstring O10) -- the
# per-charge WHY trace for the over-charge audit's continuously-visible-unit re-billing. The module itself
# writes <out>/ticks.jsonl, summary.json, summary.md, charges.jsonl, track_events.jsonl and prints one
# {"OPP_EST_AUDIT_DONE": {...}} JSON line at the end -- this chain only launches it and records exit status;
# it does not score or interpret output.
#
# Traps carried from chain_split_scalars.ps1 / chain_noise_attrib.ps1 / chain_opp_confirm.ps1 /
# chain_opp_est_audit.ps1: never `Start-Process -Wait` (it waits for descendants and hung 23 min once) -- use
# -PassThru + Wait-Process -Id; no in-script scoring (a `2>&1` inside a -File PowerShell turned stdout into
# RemoteException records and lost the report -- score from Bash afterwards); boot only through
# L63/s0/_boot.ps1, never from inside a python process tree.
...
function Run-Audit([string]$Name, [string]$OutDir, [string[]]$Extra) {
  ...
  $null = $p.Handle
  "$([DateTime]::UtcNow.ToString('s'))Z pid=$($p.Id) opp_trace $Name port=38031 out=$OutDir" | Add-Content "$E1\pids.txt"
...
function Stop-Engine {
  ...
  $null = Run-Worker 'stop --stop-vm' "$E1\status_stop_trace.txt"
...
Note "opp_trace start (entries 0:5)"

# 1. worker status; boot only if not already up
$st = Run-Worker 'status' "$E1\status_trace_pre.txt"
if ($st -match '"vm_ready":\s*true' -and $st -match '"services":\s*\[\s*true,\s*true\s*\]') { Note "services already up -- boot skipped" }
else {
  $bl = "$E1\boot_trace.log"
  $b = Start-Process powershell.exe -WindowStyle Hidden -PassThru -ArgumentList '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
         "$Repo\scratchpad\gauntlet\L63\s0\_boot.ps1" -RedirectStandardOutput $bl -RedirectStandardError "$E1\boot_trace.stderr.log"
  "$([DateTime]::UtcNow.ToString('s'))Z pid=$($b.Id) boot_trace" | Add-Content "$E1\pids.txt"
  $t0 = Get-Date
  ...
  if ($boot -notmatch 'exit=0') { Note "BOOT FAILED -- see boot_trace.log"; Stop-Engine; Note "ALL DONE (boot failed)"; exit 2 }
  Note ("boot ok: " + (($boot -split "`n" | Select-String 'attempt|exit=') -join ' | '))
}

# 2. liveness on 38031
$LOut = "$E1\liveness\p38031_trace"
...
# 3. trace_0_5 -- the arm under measurement, entries 0:5, with the charge trace on
$null = Run-Audit 'trace_0_5' "$A\trace_0_5" @('--entries', '0:5', '--charge-trace')
