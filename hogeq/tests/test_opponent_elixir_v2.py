...
O13 attempt 2 FIX 6a (verifier, LOW): hogeq's opponent_elixir.py is byte-identical to icebow's (both V1 and
this ticket's V2 addition -- confirmed via `diff`), and hogeq/tests/test_opponent_elixir.py mirrors icebow's
own test_opponent_elixir.py convention exactly (same `_FakeDB`/`_Det` shape, same `from clashrl.opponent_elixir
import ...`, run the same way via `PYTHONPATH=hogeq/src`), so this file mirrors
icebow/tests/test_opponent_elixir_v2.py verbatim -- same tests, same expectations, run against hogeq's own
copy of the module instead of icebow's.
...
class _FakeDB:
    """Same stand-in convention as hogeq/tests/test_opponent_elixir.py, extended with `speed_tiles` for R3."""
...
# Captured by loading the git-committed HEAD (64d7eca) copy of icebow's opponent_elixir.py as a separate
# module (hogeq's own copy was confirmed byte-identical to icebow's at that commit too), so this test can
# never pass by coincidence if the live import were accidentally modified.
# ------------------------------------------------------------------------------------------------------
def _load_golden_v1():
    ...
    try:
        with open(fd, "w", encoding="utf-8") as fh:
            fh.write(src)
        spec = importlib.util.spec_from_file_location("_golden_opponent_elixir_v1_hogeq", tmp)
...
class TestV1ParityUnchanged(unittest.TestCase):
    """hogeq's OpponentElixirEstimator must behave identically to the HEAD-64d7eca golden copy on 3
